...
from universal_mcp_fireflies.app import FirefliesApp

@pytest.fixture
def app_instance():
    """Provides a FirefliesApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return FirefliesApp(integration=mock_integration)

def test_universal_mcp_fireflies_app_initialization(app_instance):
    """
    Test that the FirefliesApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "fireflies", "FirefliesApp instance has unexpected name."


def test_universal_mcp_fireflies_tool_docstrings_format(app_instance):
    """
    Test that each tool method in FirefliesApp has a well-formatted docstring,
...
def test_universal_mcp_fireflies_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in FirefliesApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
